...
<div class="page-hero no-print">
    <div class="page-hero-left">
        <h1 class="page-hero-title">
            <i class="bi bi-person-gear me-2"></i>Job Assignments Report
        </h1>
        <p class="page-hero-sub">Generate and print job assignments reports for Gayatri Solar Energy</p>
    </div>
...
<div class="print-title">
    Gayatri Solar Energy — Job Assignments Report<br>
...
<div class="report-card">
    ...
    <div class="filter-section no-print">
        <form method="GET" action="{{ route('reports.job-assignments') }}">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-6 col-sm-4 col-md-2">
                    ...
                    <select name="technician_id" class="filter-control">
                ...
                <div class="col-12 col-sm-auto d-flex gap-2">
                    ...
                    <a href="{{ route('reports.job-assignments') }}" class="btn-reset">
                ...
                <tr>
                    ...
                    <td>
                        <span style="font-weight:600;color:#1F2937;">
                            {{ optional(optional($row->serviceRequest)->customer)->name ?? '—' }}
                        </span>
                    </td>
                    <td>{{ optional(optional($row->serviceRequest)->service)->service_name ?? '—' }}</td>
                    <td>{{ optional($row->technician)->name ?? '—' }}</td>
                    <td>{{ $row->assigned_date ? $row->assigned_date->format('d M Y') : '—' }}</td>
                    <td>{{ $row->scheduled_date ? $row->scheduled_date->format('d M Y') : '—' }}</td>
                    <td>
                        @if($row->scheduled_time)
                            {{ \Carbon\Carbon::parse($row->scheduled_time)->format('h:i A') }}
                        @else
                            —
                        @endif
                    </td>
